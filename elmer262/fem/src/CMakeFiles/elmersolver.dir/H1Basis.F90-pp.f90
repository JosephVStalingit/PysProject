# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/H1Basis.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/H1Basis.F90"
!
...
!
!
# 35 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/H1Basis.F90"
...
# 44 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/H1Basis.F90" 2
...
CONTAINS
  ...
  SUBROUTINE H1Basis_GetEdgeDirection(ecode, nedges, globalind, direction)
    ...
    INTEGER, DIMENSION(:), POINTER , INTENT(IN) :: globalind
...
!DIR$ LOOP COUNT MAX=12
  ...
  SUBROUTINE H1Basis_GetTetraEdgeDirection(ttype, direction)
    ...
    ! Local variables
    INTEGER, DIMENSION(:), POINTER  :: flatdir
...
!DIR$ LOOP COUNT MAX=6
  ...
  SUBROUTINE H1Basis_GetEdgeMap(ecode, map)
    ...
    ! Local variables
    INTEGER, DIMENSION(:), POINTER  :: flatmap
...
!DIR$ ASSUME_ALIGNED u:64, v:64, grad:64
  ...
  SUBROUTINE H1Basis_TriangleEdgeP(nvec, u, v, pmax, nbasismax, fval, nbasis, edgedir)
    ...
    INTEGER, INTENT(IN) :: nvec
    REAL(KIND=dp), DIMENSION(VECTOR_BLOCK_LENGTH), INTENT(IN) :: u, v
    INTEGER, DIMENSION(:) , INTENT(IN) :: pmax
    INTEGER, INTENT(IN) :: nbasismax
    REAL(KIND=dp), DIMENSION(VECTOR_BLOCK_LENGTH,nbasismax), INTENT(INOUT) :: fval
    INTEGER, INTENT(INOUT) :: nbasis
    INTEGER, DIMENSION(:,:) , INTENT(IN) :: edgedir
...
!DIR$ ASSUME_ALIGNED u:64, v:64, grad:64
    ...
    DO i=1,3
      ...
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Lb, vPhi, dVPhi)
        DO k=1,nvec
          La = H1Basis_TriangleL(edgedir(1,i),u(k),v(k))
          Lb = H1Basis_TriangleL(edgedir(2,i),u(k),v(k))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, grad:64
    ...
    IF (.NOT. PRESENT(localnumbers)) THEN
      ...
      DO i = 0,pmax-3
        DO j = 1,pmax-i-2
          !DIR$ IVDEP ! PRIVATE(La, Lb, Lc, Lb_Lai, Lc_1n)
          DO k=1,nvec
            La = H1Basis_TriangleL(1,u(k),v(k))
            Lb = H1Basis_TriangleL(2,u(k),v(k))
            Lc = H1Basis_TriangleL(3,u(k),v(k))
    ...
    ELSE
      ...
      DO i=0,pmax-3
        DO j=1,pmax-i-2
          !DIR$ IVDEP ! PRIVATE(La, Lb, Lc, Lb_La, Lc_1, a, b)
          DO k=1,nvec
            La=H1Basis_TriangleL(localnumbers(1),u(k),v(k))
            Lb=H1Basis_TriangleL(localnumbers(2),u(k),v(k))
            Lc=H1Basis_TriangleL(localnumbers(3),u(k),v(k))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, grad:64
    ...
    DO i=1,4
      ...
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Lb, Phi, dPhi)
        DO k=1,nvec
          La = H1Basis_QuadL(edgedir(1,i), u(k), v(k))
          Lb = H1Basis_QuadL(edgedir(2,i), u(k), v(k))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, grad:64
    ...
    DO i=1,4
      ...
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Lb, Phi, dPhi, Na, Nb, dNa, dNb)
        DO k=1,nvec
          La = H1Basis_QuadL(node1, u(k), v(k))
          Lb = H1Basis_QuadL(node2, u(k), v(k))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, fval:64
    ...
    ELSE
      DO i=0,pmax-2
        DO j=1,pmax-1
          !DIR$ IVDEP ! PRIVATE(La,Lb,Lc,Pa,Pb)
          DO k=1,nvec
            ! Directed quad bubbles
            La = H1Basis_QuadL(localNumbers(1),u(k),v(k))
            Lb = H1Basis_QuadL(localNumbers(2),u(k),v(k))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, grad:64
    ...
    ELSE
      ...
      DO i=0,pmax-2
        DO j=1,pmax-1
          !DIR$ IVDEP ! PRIVATE(La,Lb,Lc,Legi,Legj,dLegi,dLegj,Pa,Pb,dPa,dPb)
          DO k=1,nvec
            La = H1Basis_QuadL(localNumbers(1),u(k),v(k))
            Lb = H1Basis_QuadL(localNumbers(2),u(k),v(k))
            Lc = H1Basis_QuadL(localNumbers(4),u(k),v(k))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
  ...
  SUBROUTINE H1Basis_TetraFaceP(nvec, u, v, w, pmax, nbasismax, fval, nbasis, facedir)
    ...
    REAL(KIND=dp), DIMENSION(VECTOR_BLOCK_LENGTH,nbasismax), INTENT(INOUT) :: fval
    INTEGER, INTENT(INOUT) :: nbasis
    INTEGER, DIMENSION(:,:) , INTENT(IN) :: facedir
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, fval:64
    ...
    DO i=0,pmax-4
      DO j=0,pmax-i-4
        DO k=1,pmax-i-j-3
          !DIR$ IVDEP ! PRIVATE(L1,L2,L3,L4,L2_L1,L3_1,L4_1)
          DO l=1,nvec
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=0,pmax-4
      DO j=0,pmax-i-4
        DO k=1,pmax-i-j-3
          !DIR$ IVDEP ! PRIVATE(L1,L2,L3,L4,L2_L1,L3_1,L4_1,a,b,c)
          DO l=1,nvec
            L1 = H1Basis_TetraL(1,u(l),v(l),w(l))
            L2 = H1Basis_TetraL(2,u(l),v(l),w(l))
            L3 = H1Basis_TetraL(3,u(l),v(l),w(l))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=1,6
      ...
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Lb, Na, Nb, vPhi, dVPhi, NaNb)
        DO k=1,nvec
          La = H1Basis_WedgeL(edgedir(1,i), u(k), v(k))
          Lb = H1Basis_WedgeL(edgedir(2,i), u(k), v(k))
          Na = H1Basis_WedgeH(edgedir(1,i), w(k))
    ...
    DO i=7,9
      ...
      dNb = H1Basis_dWedgeH(edgedir(2,i))
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Na, Nb, Phi, dPhi)
        DO k=1,nvec
          La = H1Basis_WedgeL(edgedir(1,i), u(k), v(k))
          Na = H1Basis_WedgeH(edgedir(1,i), w(k))
          Nb = H1Basis_WedgeH(edgedir(2,i), w(k))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=1,2
      ...
      DO j=0,pmax(i)-3
        DO k=1,pmax(i)-j-2
          !DIR$ IVDEP ! PRIVATE(La, Lb, Lc, Na, LegPLbLa, LegP2Lc1, cNa)
          DO l=1,nvec
            La = H1Basis_WedgeL(facedir(1,i), u(l), v(l))
            Lb = H1Basis_WedgeL(facedir(2,i), u(l), v(l))
            Lc = H1Basis_WedgeL(facedir(3,i), u(l), v(l))
    ...
    DO i=3,5
      ...
      DO j=2,pmax(i)-2
        DO k=1,pmax(i)-j-1
          IF (nonpermuted) THEN
            !DIR$ IVDEP ! PRIVATE(La,Lb,Na,Nc,vPhi,Phi)
            DO l=1,nvec
              La = H1Basis_WedgeL(facedir(1,i), u(l), v(l))
              Lb = H1Basis_WedgeL(facedir(2,i), u(l), v(l))
              Na = H1Basis_WedgeH(facedir(1,i), w(l))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, fval:64
    ...
    DO i=0,pmax-5
      DO j=0,pmax-5-i
        DO k=1,pmax-4-i-j
          !DIR$ IVDEP ! PRIVATE(L1, L2, L3, L2_L1, L3_1)
          DO l=1,nvec
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64      
    ...
    DO i=0,pmax-5
      DO j=0,pmax-5-i
        DO k=1,pmax-4-i-j
          !DIR$ IVDEP ! PRIVATE(L1, L2, L3, Legi, Legj, phiW, L2_L1, L3_1)
          DO l=1,nvec
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=1,6
      ...
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Lb, Na, Nb, dNa, dNb, vPhi, dVPhi)
        DO k=1,nvec
          La = H1Basis_WedgeL(node1, u(k), v(k))
          Lb = H1Basis_WedgeL(node2, u(k), v(k))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, fval:64
    ...
    DO i=3,5
      ...
      DO j=0,pmax(i)-2
        DO k=1,pmax(i)-1
          IF (nonpermuted) THEN
            !DIR$ IVDEP ! PRIVATE(La,Lb,Na,Nb,Lha,Lhc)
            DO l=1,nvec
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=3,5
      ...
      DO j=0,pmax(i)-2
        DO k=1,pmax(i)-1
          IF (nonpermuted) THEN
            !DIR$ IVDEP ! PRIVATE(La,Lb,Lha,Lhc,Na,Nb,dNa,dNb,Legj,Legk,dLegj,dLegk)
            DO l=1,nvec
              La = H1Basis_WedgeL(node1, u(l), v(l))
              Lb = H1Basis_WedgeL(node2, u(l), v(l))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, fval:64
    ...
    DO i=0,pmax-3
      DO j=0,pmax-3-i
        DO k=1,pmax-1
          !DIR$ IVDEP ! PRIVATE(L1, L2, L3, s,t)
          DO l=1,nvec
            L1 = H1Basis_WedgeL(1,u(l),v(l))
            L2 = H1Basis_WedgeL(2,u(l),v(l))
            L3 = H1Basis_WedgeL(3,u(l),v(l))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64      
    ...
    DO i=0,pmax-3
      DO j=0,pmax-3-i
        DO k=1,pmax-1
          !DIR$ IVDEP ! PRIVATE(L1, L2, L3, Legi, Legj, Legk, dLegi, dLegj, dLegk, s,t,ds,dt)
          DO l=1,nvec
...
!!!!DIR$ ASSUME_ALIGNED u:64, v:64, w:64
    ...
    DO i=1,4 ! square face
      ...
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Lb, Na, Nb, dNa, dNb, Phi, dPhi)
        DO k=1,nvec
          Na = N(k,node1)
          Nb = N(k,node2)
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, fval:64
    ...
    DO i=2,5 ! tri face
      ...
      DO j=0,pmax(i)-3
        DO k=1,pmax(i)-2-j
          !DIR$ IVDEP ! PRIVATE(La, Lb, Lc, Pa, Pb, Pc)
          DO l=1,nvec
            Pa = fval(l,node1)
            Pb = fval(l,node2)
            Pc = fval(l,node3)
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64
    ...
    DO i=1,1 ! square face
      ...
      DO j=0,pmax(i)-2
        DO k=1,pmax(i)-1
          !DIR$ IVDEP ! PRIVATE(La, Lb, Lc, Pa, Pb, dPa, dPb, Legi, Legj, dLegi, dLegj)
          DO l=1,nvec
            Pa = n(l,node1)
            Pb = n(l,node3)
    ...
    DO i=2,5 ! tri face
      ...
      DO j=0,pmax(i)-3
        DO k=1,pmax(i)-2-j
          !DIR$ IVDEP ! PRIVATE(La, Lb, Lc, Pa, Pb, Pc, dPa, dPb, dPc, Legi, Legj, dLegi, dLegj)
          DO l=1,nvec
            Pa = n(l,node1)
            Pb = n(l,node2)
            Pc = n(l,node3)
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, fval:64
    ...
    DO i=0,pmax-3
      DO j=0,pmax-i-3
        DO k=1,pmax-i-j-2
          nbasis = nbasis + 1
          !DIR$ IVDEP ! PRIVATE(Pa, Pb, Pc, Legi, Legj, Legk,s)
          DO l=1,nvec
             s = w(l) / SQRT(2._dp)
             Pa = fval(l,1)
             Pb = fval(l,3)
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64
    ...
    DO i=0,pmax-3
      DO j=0,pmax-i-3
        DO k=1,pmax-i-j-2
          nbasis = nbasis + 1
          !DIR$ IVDEP ! PRIVATE(Pa, Pb, Pc, Legi, Legj, Legk, dPa, dPb, dPc, dLegi, dLegj, dLegk, s)
          DO l=1,nvec
             s = a * w(l)
             Pa = N(l,1)
             Pb = N(l,3)
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=1,12
      ...
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Lb, Aa, Ba, Phi, dPhi)
        DO k=1,nvec
          La=H1Basis_BrickL(edgedir(1,i), u(k), v(k), w(k))
          Lb=H1Basis_BrickL(edgedir(2,i), u(k), v(k), w(k))
          CALL H1Basis_BrickEdgeL(i, u(k), v(k), w(k), Aa, Ba)
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=1,6
      ...
      DO j=2,pmax(i)
        DO k=1,pmax(i)-j-1
          !DIR$ IVDEP ! PRIVATE(La, Lb, Lc, Ld, PhiLbLa, PhiLdLa, LaLbLcLd)
          DO l=1,nvec
            La=H1Basis_BrickL(facedir(1,i), u(l), v(l), w(l))
            Lb=H1Basis_BrickL(facedir(2,i), u(l), v(l), w(l))
            Lc=H1Basis_BrickL(facedir(3,i), u(l), v(l), w(l))
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=2,pmax-4
      DO j=2,pmax-i-2
        DO k=1,pmax-i-j-1
          !DIR$ IVDEP ! PRIVATE(phiU, phiV, phiW)
          DO l=1,nvec
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=1,12
      ...
      DO j=1,pmax(i)-1
        !DIR$ IVDEP ! PRIVATE(La, Lb, Na, Nb, Phi, dPhi)
        DO k=1,nvec
          La = Lp(k,node1)
          Lb = Lp(k,node2)
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, fval:64
    ...
    DO i=1,6
      ...
      DO j=0,pmax(i)-2
        DO k=1,pmax(i)-1
          !DIR$ IVDEP ! PRIVATE(Na,Nb,La, Lb, Ld)
          DO l=1,nvec
            Na = fval(l,node1)
            Nb = fval(l,node3)
            La = Lp(l,node1)
...
!DIR$ ASSUME_ALIGNED u:64, v:64, w:64, grad:64
    ...
    DO i=1,6
      ...
      DO j=0,pmax(i)-2
        DO k=1,pmax(i)-1
          !DIR$ IVDEP ! PRIVATE(La, Lb, Ld, PhiU, PhiV, Na, Nb, dNa, dNb)
          DO l=1,nvec
