# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/IterSolve.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/IterSolve.F90"
!
...
# 38 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/IterSolve.F90" 2
...
# 124 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/IterSolve.F90"
...
!------------------------------------------------------------------------------
    INTEGER :: ipar(*)
    REAL(KIND=dp) :: u(ipar(3)), v(ipar(3))
    INTEGER :: i
!------------------------------------------------------------------------------
    !$OMP PARALLEL DO
    DO i=1,ipar(3)
...
!------------------------------------------------------------------------------
    INTEGER :: ipar(*)
    COMPLEX(KIND=dp) :: u(ipar(3)), v(ipar(3))
!------------------------------------------------------------------------------
    u = v 
!------------------------------------------------------------------------------
...
# 207 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/IterSolve.F90"
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
    ...
    REAL(KIND=dp), DIMENSION(:), TARGET  :: x,b
...
!------------------------------------------------------------------------------
    ...
    IF( PseudoComplexSystem ) THEN
      ipar(7) = 1     
      IF ( ListGetLogical( Params,'Block Split Complex',Found ) ) ipar(7) = 2
    END IF
    Internal = .FALSE.
    ...
    CASE (ITER_GMRES)
      ipar(15) = ListGetInteger( Params,&
          'Linear System GMRES Restart',  GotIt ) 
      IF ( .NOT. GotIT ) ipar(15) = 10
      ipar(4) = 7 + ipar(15)
    ...
    CASE (ITER_SGS)
      ipar(4) = 1
      dpar(3) = ListGetConstReal( Params,'SGS Overrelaxation Factor',&
          GotIt,minv=0.0_dp,maxv=2.0_dp)
      IF(.NOT. GotIt) dpar(3) = 1.8
      Internal = .TRUE.
    ...
    CASE (ITER_BICGSTABL)
      ipar(4) = 1
      ipar(16) = ListGetInteger( Params,'BiCGstabl polynomial degree',&
          GotIt,minv=2)
      IF(.NOT. GotIt) ipar(16) = 2
      Internal = .TRUE.

    CASE (ITER_IDRS)
      ipar(4) = 1
      ipar(18) = ListGetInteger( Params,'IDRS parameter',GotIt,minv=1)
      IF(.NOT. GotIt) ipar(18) = 4
      Internal = .TRUE.
    
    CASE (ITER_MPRGP)
      ipar(4) = 1
      Internal = .TRUE.
      dpar(6) = ListGetConstReal( Params, 'Linear System MPRGP Gamma', GotIt )
      IF(.NOT. GotIt) dpar(6) = 1.0_dp
      dpar(7) = ListGetConstReal( Params, 'Linear System MPRGP TolFactor', GotIt )
      IF(.NOT. GotIt) dpar(7) = 5.0_dp
...
!------------------------------------------------------------------------------
    ...
    ipar(5)  = ListGetInteger( Params, &
        'Linear System Residual Output', GotIt )
    IF ( .NOT.Gotit ) ipar(5) = 1
    
    IF ( Parenv % myPE /= 0 ) ipar(5)=0
    
    ipar(10) = ListGetInteger( Params, &
        'Linear System Max Iterations', minv=1 )
    
    ipar(11) = ListGetInteger( Params, &
    ...
    dpar(1) = ListGetCReal( Params, &
        'Linear System Convergence Tolerance' )
    
    dpar(2) = ListGetCReal( Params, &
        'Linear System Divergence Limit', GotIt)
    IF(.NOT. GotIt) dpar(2) = 1.0d20
    
    IF( ListGetLogical( Params,'Linear System Robust',GotIt) ) THEN
      ipar(26) = 1
      dpar(3) = ListGetCReal( Params,'Linear System Robust Tolerance',GotIt)
      IF(.NOT. GotIt ) dpar(3) = dpar(1)**(2.0/3.0)
      dpar(5) = ListGetCReal( Params,'Linear System Robust Limit',GotIt)
      IF(.NOT. GotIt ) dpar(5) = SQRT( dpar(1) )      
      dpar(4) = ListGetCReal( Params,'Linear System Robust Margin',GotIt)
      IF(.NOT. GotIt ) dpar(4) = 1.1_dp
      ipar(27) = ListGetInteger( Params,'Linear System Robust Max Iterations',GotIt)
      IF(.NOT. GotIt ) ipar(27) = ipar(10) / 2
      ipar(29) = ListGetInteger( Params,'Linear System Robust Start Iteration',GotIt)
      IF(.NOT. GotIt ) ipar(29) = 1
    ELSE
      ipar(26) = 0
    END IF
...
!         IF ( dotProc  == 0 ) dotProc = AddrFunc(ddot)
        ...
        IF( ipar(5) == 0) ipar(5) = HUGE( ipar(5) )        
      END IF
...
!------------------------------------------------------------------------------
    ...
    IF ( ComplexSystem ) THEN
      ...
      ALLOCATE(xC(ipar(3)), bC(ipar(3)), STAT=astat)
      IF (astat /= 0) THEN
    ...
    IF ( ComplexSystem ) ipar(3) = ipar(3) * 2
    ...
    IF ( ipar(30) == 1 ) THEN
      ...
      END IF
    ELSE
      CALL Info('IterSolve','Returned return code: '//I2S(ipar(30)),Level=15)
      IF( ipar(30) == 3 ) THEN
        CALL NumericalError( 'IterSolve', 'System diverged over maximum tolerance.')
      ELSE IF( ipar(30) == 2 ) THEN                
