# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/SParIterComm.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/SParIterComm.F90"
!
...
!
!
# 36 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/SParIterComm.F90"
...
# 46 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/SParIterComm.F90" 2
...
# 84 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/SParIterComm.F90"
  INCLUDE "mpif.h"
...
!-----------------------------------------------------------------------
    ...
    ierr = 0
# 209 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/SParIterComm.F90"
...
# 241 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/SParIterComm.F90"
    ...
    CALL MPI_COMM_SPLIT(MPI_COMM_WORLD,0,&
...
!-----------------------------------------------------------------------
  SUBROUTINE AddToCommonList( list, val )
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
  SUBROUTINE AddToCommonList2( list, val )
!-----------------------------------------------------------------------
! Helper subroutine for SParIterGlobalNumbering
! Adds integer Entry to integer pointer List(:)
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
    ...
    INTEGER :: itmp,i
!-----------------------------------------------------------------------
    IF (.NOT. ASSOCIATED(list % clist))THEN
      ALLOCATE(list % clist(64))
      List % Count = 0
    ELSE IF ( list % count >= SIZE(list % clist) ) THEN
      ALLOCATE(ptmp(list % count+64))
      ptmp(1:list % count) = list % clist(1:list % count)
      DEALLOCATE(list % clist)
      List % clist => ptmp
    END IF
...
!-----------------------------------------------------------------------
     ...
     TYPE(CommonList_t) :: commonlist, cgorder
     TYPE Node_t
        TYPE(CommonList_t) :: ElementIndexes
     END TYPE Node_t
     TYPE(Node_t), POINTER :: Node(:)

     TYPE Request_t
        TYPE(CommonList_t) :: DATA
     END TYPE Request_t
...
!    END DO
     ...
     DO i = 1,Mesh % NumberOfBulkElements
        ...
        DO j = 1,SIZE( Element % NodeIndexes )
           ...
           CALL AddToCommonList2( Node(k) % ElementIndexes, i )
        END DO
     END DO
     !
...
!       commonlist => NULL() ! intersection of list1 and list2
        ...
        !--------------------------------------------------
        DO j = 1,  Node(i) % ElementIndexes % Count
           Element => Mesh % Elements( Node(i) % ElementIndexes % clist(j) )
           !
...
!          IF( .NOT.ASSOCIATED(commonlist) ) CYCLE
           ...
           !-------------------------------------------------------------------
           ALLOCATE( gindices( commonlist % count ) )
           gindices = commonlist % clist(1:commonlist % count)
!          DEALLOCATE( commonlist )
...
!       Request2(i) % DATA => NULL() ! buffer for data exchange
        Request1(i) % data % count = 0
        Request2(i) % data % count = 0
     END DO
     !
     ...
     DO i = 1, ParEnv % PEs
        ...
        DO l=1,DataSize
           IF( gindices(3*l-2)>0 ) THEN
              ...
              !----------------------------------------------------
              CALL AddToCommonList2( Request1(i) % DATA, gindices(3*l-2) )  ! i-node
              CALL AddToCommonList2( Request1(i) % DATA, gindices(3*l-1) )  ! parent 1
              CALL AddToCommonList2( Request1(i) % DATA, gindices(3*l-0) )  ! parent 2
           END IF
        END DO
        DEALLOCATE( gindices )
     END DO
     ...
     DO i = n, Mesh % NumberOfNodes
       IF( Mesh % ParallelInfo % GlobalDOFs(i) <= 0 ) THEN
          ...
          DO j = 1,SIZE( Mesh % ParallelInfo % NeighbourList(i) % Neighbours )
             ...
             CALL AddToCommonList2( Request2(k+1) % DATA, TmpArray(1) )
             CALL AddToCommonList2( Request2(k+1) % DATA, TmpArray(2) )
             CALL AddToCommonList2( Request2(k+1) % DATA, TmpArray(3) )
          END DO
        END IF
     END DO
...
!       IF( ASSOCIATED( Request2(i) % DATA ) ) DataSize = SIZE( Request2(i) % DATA )
        DataSize = Request2(i) % Data % Count
!ddd = ddd+DataSize
        ...
        CALL MPI_BSEND( Request2(i) % DATA % clist, DataSize, MPI_INTEGER, i-1, 950, ELMER_COMM_WORLD, ierr)
     END DO
     ...
     DO i = 1, ParEnv % PEs
        ...
        DO q = 1, DataSize/3
           ...
           DO j = n, Mesh % NumberOfNodes
              ...
              mm = 0
              nn = 0
              DO k = 1,Node(j) % ElementIndexes % count
                 l = Node(j) % ElementIndexes % clist(k)
           ...
           IF( .NOT.(mm==1 .AND. nn==1) ) THEN
              ...
              CALL AddToCommonList2( Request1(i) % DATA, TmpArray(1) )
              CALL AddToCommonList2( Request1(i) % DATA, TmpArray(2) )
              CALL AddToCommonList2( Request1(i) % DATA, TmpArray(3) )
           END IF
        END DO
        DEALLOCATE( IntArray )
     END DO
...
!       IF( ASSOCIATED( Request1(i) % DATA ) ) DataSize = SIZE( Request1(i) % DATA )
        DataSize = Request1(i) % data % count
        CALL MPI_BSEND( DataSize, 1, MPI_INTEGER, i-1, 700, ELMER_COMM_WORLD, ierr)
        IF( DataSize < 1 ) CYCLE
        CALL MPI_BSEND( Request1(i) % DATA % clist, DataSize, MPI_INTEGER, i-1, 1800, ELMER_COMM_WORLD, ierr )
     END DO
...
!                gorder => NULL()
                 ...
                 DO k = 1,SIZE( Mesh % ParallelInfo % NeighbourList(j) % Neighbours )
                    ...
                    IF( l == i-1 ) CYCLE
                    CALL AddToCommonList2(cgorder, l)
                 END DO
                 DEALLOCATE( Mesh % ParallelInfo % NeighbourList(j) % Neighbours )
                 ALLOCATE(IntCnts(cgorder % count))
                 intCnts = cgorder % clist(1:cgorder % count)
!                IF (ASSOCIATED(gorder)) DEALLOCATE( gorder )
                 cgorder % count = 0
...
!      DEALLOCATE( Node(i) % ElementIndexes )
       IF (ASSOCIATED( Node(i) % ElementIndexes % Clist)) &
          DEALLOCATE(Node(i) % ElementIndexes % Clist)
     END DO
     DEALLOCATE(Node)

     DO i = 1, ParEnv % PEs
       IF(ASSOCIATED(Request1(i) % Data % Clist)) &
         DEALLOCATE(Request1(i) % Data % Clist)

       IF(ASSOCIATED(Request2(i) % Data % Clist)) &
         DEALLOCATE(Request2(i) % Data % Clist)
!      IF( ASSOCIATED(Request1(i) % DATA) ) DEALLOCATE(Request1(i) % DATA)
!      IF( ASSOCIATED(Request2(i) % DATA) ) DEALLOCATE(Request2(i) % DATA)
     END DO
!    DEALLOCATE( Request1, Request2 )
     ...
     !-------------------------------------------------------------------
!
!    Sort our own nodes according to ascending global order:
!    -------------------------------------------------------
...
!*********************************************************************
  ...
  DO i=1,n
    IF ( send_size(i) > 0 ) &
      ALLOCATE(send_buf(i) % ind(send_size(i)),send_buf(i) % ivec(send_size(i)))
  END DO
  ...
  DO i = 1, SourceMatrix % NumberOfRows
    DO j=1,SIZE(ParallelInfo % NeighbourList(i) % Neighbours)
      ...
      IF ( owner /= ParEnv % MyPE .AND. ParEnv % Active(owner+1) ) THEN
         owner = perm(owner)
         send_size(owner) = send_size(owner) + 1
         send_buf(owner) % ivec(send_size(owner)) = SourceVec(i)
         send_buf(owner) % ind(send_size(owner)) = ParallelInfo % GlobalDOFs(i)
      END IF
    END DO
  END DO
...
! --------------------------------------------------------------------
  ...
  DO i = 1, n
     ...
     IF ( datalen > 0 ) THEN
        req_cnt = req_cnt + 1
        ALLOCATE( recv_buf(i) % ivec(datalen) )
        CALL MPI_iRECV( recv_buf(i) % ivec, datalen, MPI_INTEGER, &
             sproc, 3002, ELMER_COMM_WORLD, requests(req_cnt), ierr )
     END IF
  END DO
  ...
  DO i = 1, n
    ...
    datalen = send_size(i)
    IF ( datalen > 0 ) THEN
       CALL MPI_BSEND( send_buf(i) % ivec, datalen, &
          MPI_INTEGER, destproc, 3002, ELMER_COMM_WORLD, ierr )
    END IF
  END DO
  CALL MPI_WaitAll( req_cnt, requests, MPI_STATUSES_IGNORE, ierr )
...
!         Ind = SourceMatrix % Perm(Ind)
          IF ( Ind > 0 ) THEN
             SELECT CASE(oper)
             CASE(0)
               SourceVec(Ind) = SourceVec(Ind) + recv_buf(i) % ivec(j)
             CASE(1)
               SourceVec(Ind) = MIN(SourceVec(Ind),recv_buf(i) % ivec(j))
             CASE(2)
               SourceVec(Ind) = MAX(SourceVec(Ind),recv_buf(i) % ivec(j))
             END SELECT
          END IF
       END IF
    END DO
  ...
  DO i=1,n
    IF (send_size(i)>0) DEALLOCATE(send_buf(i) % Ind, send_buf(i) % iVec)
    IF (recv_size(i)>0) DEALLOCATE(recv_buf(i) % Ind, recv_buf(i) % iVec)
  END DO
  DEALLOCATE( recv_buf, send_buf, recv_size, send_size, requests, neigh, perm )

!*********************************************************************
END SUBROUTINE ExchangeSourceVecInt
!*********************************************************************
...
SUBROUTINE Recv_LocIf_Wait( SplittedMatrix, ndim, v, n, neigh, &
  ...
  DO j=1,active_cnt
    ...
    END DO
  END DO
# 4714 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/SParIterComm.F90"
!*********************************************************************
...
!*********************************************************************
SUBROUTINE SParActiveSUMInt(tsum, oper)
   INTEGER :: oper
   INTEGER :: tsum
!*********************************************************************
   INTEGER :: ierr, comm, nact
   ...
   SELECT CASE(oper)
   CASE(0)
     CALL MPI_ALLREDUCE( isum, tsum, 1, MPI_INTEGER, MPI_SUM, comm, ierr )
   CASE(1)
     CALL MPI_ALLREDUCE( isum, tsum, 1, MPI_INTEGER, MPI_MIN, comm, ierr )
   CASE(2)
     CALL MPI_ALLREDUCE( isum, tsum, 1, MPI_INTEGER, MPI_MAX, comm, ierr )
  END SELECT
!*********************************************************************
