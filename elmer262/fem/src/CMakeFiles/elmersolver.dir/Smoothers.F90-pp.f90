# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/Smoothers.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/Smoothers.F90"
!
...
!
!
# 36 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/src/Smoothers.F90"
...
!      ---------------------------
      ...
      END IF
      IF ( .NOT. Found ) THEN
        IterMethod = ListGetString( Solver % Values, 'Linear System Iterative Method', Found )
      END IF
...
!------------------------------------------------------------------------------
        ...
        ELSE
          L = SIZE(b) == A % NumberOfRows/2
          IF ( PRESENT( Update ) ) THEN
            CALL ParallelMatrixVectorC( A,x,b,Update,ZeroNotOwned=L )
          ELSE
            CALL ParallelMatrixVectorC( A,x,b,ZeroNotOwned=L)
          END IF
          CALL SParIterActiveBarrier()
        END IF
!------------------------------------------------------------------------------
...
!$OMP PARALLEL DO shared(n,x,b,r,Values,Diag) private(k,v)
          DO j=1,n/2
            k = Diag(2*j-1)
            v = CMPLX(Values(k), -Values(k+1), KIND=dp)
            r(j) = (b(j)-r(j)) / v
            x(j) = x(j) + w*r(j)
          END DO
!$OMP END PARALLEL DO
...
!------------------------------------------------------------------------------
        ...
        INTEGER, POINTER  :: Cols(:),Rows(:),ColsB(:),RowsB(:),DiagB(:),&
            TopPointer(:),BotPointer(:)
        REAL(KIND=dp), POINTER  :: Values(:),ValuesB(:)
