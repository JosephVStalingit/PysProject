# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/tests/SecondOrderEdgeElement2D/EdgeElementTest.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/tests/SecondOrderEdgeElement2D/EdgeElementTest.F90"
...
!       rotu(2) = SUM( Load(1,np+1:nd) * CurlEBasis(1:nd-np,2) )
       ...
       IF (UseEnergyNorm) THEN
          ! Energy norm 
          SolNorm = SolNorm + (SUM( Sol(1:3) * Sol(1:3) ) + SUM( rotsol(1:3) * rotsol(1:3) )) * detJ * IP % s(t)
          EK = EK + (SUM( e(1:3) * e(1:3) ) + SUM( rote(1:3) * rote(1:3) )) * detJ * IP % s(t)
       ELSE
          ! L2 norm
