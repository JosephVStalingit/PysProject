# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/tests/SecondOrderEdgeElement3D/EdgeElementTest.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "C:/Users/JosephVStalin/Desktop/PysProject/elmerfem-release-26.2.1/fem/tests/SecondOrderEdgeElement3D/EdgeElementTest.F90"
...
!------------------------------------------------------------------------------
  ...
  REAL(KIND=dp) :: Norm, u, v, w, Err, EK, SolNorm, MatPar = 1.0d-0
    ...
    DO t=1,IP % n
       ...
       DO p = 1,nd-np
          ...
          FORCE(i) = FORCE(i) +  (0.0d0) * EBasis(p,1) * detJ * IP % s(t) + &
               ...
               1.0d0/6.0d0*(yq*xq**3+xq*yq**3) * EBasis(p,3) * detJ * IP % s(t) + &
               MatPar * (1.0d0/6.0d0*xq**3+xq*yq**2/2.0d0) * CurlEBasis(p,1) * detJ * IP % s(t) + &
               MatPar * (-1.0d0/6.0d0*yq**3-yq*xq**2/2.0d0) * CurlEBasis(p,2) * detJ * IP % s(t) + &
...
!--------------------------------------------------------------------------------
    ...
    DO t=1,IP % n
       ...
       IF (PiolaVersion) THEN
          stat = EdgeElementInfo( Element, Nodes, IP % U(t), IP % V(t), &
               IP % W(t), F, G, detJ, Basis, EBasis, CurlEBasis, ApplyPiolaTransform = .TRUE., BasisDegree=2)
       ELSE
          stat = ElementInfo( Element, Nodes, IP % U(t), IP % V(t), &
               IP % W(t), detJ, Basis, dBasisdx )
          CALL GetEdgeBasis(Element, EBasis, CurlEBasis, Basis, dBasisdx)
       ...
       sol(1) = 0.0d0
       sol(2) = 0.0d0
       sol(3) = 1.0d0/6.0d0*(yq*xq**3 + xq*yq**3)
       rotsol(1:3) = 0.0d0
       rotsol(1) = rotsol(1) + 1.0d0/6.0d0*xq**3+xq*yq**2/2.0d0
       rotsol(2) = rotsol(2) - 1.0d0/6.0d0*yq**3-yq*xq**2/2.0d0
       e(:) = sol(:) - u(:)  
       rote(:) = rotsol(:) - rotu(:)
