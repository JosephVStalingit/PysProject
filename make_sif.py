...
_RE_DIRECT = re.compile(
    r"(?m)^(?P<ind>\s*)Linear System Solver\s*=\s*Direct\s*\r?\n"
    r"\s*Linear System Direct Method\s*=\s*UMFPACK\s*\r?\n"
    r"\s*Linear System Convergence Tolerance\s*"
    r"=\s*[0-9.eE+-]+\s*\r?\n"
    r"\s*Linear System Max Iterations\s*=\s*[0-9]+\s*\r?\n")
...
Solver 8
  ! === SOLVER 8: WIRE-DIRECTION SOLVER ===
  ! Default: scalar W via WPotentialSolver/Wsolve.  When --path coilsolver
  ! is on, this entire block is REPLACED in place by the CoilSolver /
  ! W-vector implementation (see _apply_coilsolver_path, hpc/notes.md 20.2).
  !
...
#
# --path coilsolver keys (Coil Use W Vector, Coil Normal, Resistance,
# ...) sit on Component 1 alongside the --path wsolve keys (Stranded
# Coil N_j).  They are Unlisted keywords on the Wsolve path and produce
# a one-line warning per SolveLoad -- accepted noise.  On the
# CoilSolver path they ARE read by Elmer; see the --path flag below.
_COMPONENT_BLOCK = """
! ============================================================
...
def _insert_coil_solvers(sif: str) -> str:
    ...
    # class MUST be \s and not \s: \s matches newlines too, which
    ...
    sif, n = re.subn(r'(?m)^(\s*Active Solvers\(\d+\)\s*=\s*)[0-9 \t]+$',
    ...
    sif, n = re.subn(
        r'(?m)^(\s*Active Solvers\()\d+(\)\s*=\s*5 6 7 8 1 9 2 3 10 11\s*$)',
...
def _apply_coilsolver_path(sif: str, curve: dict, cfg: dict) -> str:
    """Apply the four SIF rewrites the CoilSolver / W-vector path needs.

    Called from patch_circuit_closed() only when coil_path == "coilsolver".
    Each rewrite has a known-on-paper reason; see hpc/notes.md 20.2.

    1. Replace Solver 8 in place (WPotentialSolver -> CoilSolver, drop
       `Variable = W`, add CoilSolver normalisation keys).
    2. Trim Equation 1 Active Solvers (remove 8), move Body 2 and Body 3
       onto a new Equation 2 (same Active Solvers list, minus 8).
    3. Add `Wire direction = Logical True` to Equation 1 (logical key the
       solver's Equation string must be findable as, MainUtils.F90:1633).
    4. Strip `Coil Start = Logical True` and `Coil End = Logical True`
       from BCs 3 and 4 (CoilSolver.F90:130-152 owns these and refuses
       them if pre-set).
    """
    # ---- (1) swap Solver 8 ----
    # Match the current Solver 8 block (starts after the ! === SOLVER 8 ===
    # comment added in this revision).
    # NOTE: the replacement string starts with "Solver 8\n" because the
    # upstream match-consumes that line and the replacement string only
    # carries what came AFTER it.  Without this, the resulting SIF has
    # "Exec Solver = ..." floating in space -- which Elmer's parser then
    # mis-reads as a new (unknown) section.  (Job 122353641 died with
    # `Unknown input section name: exec solver` for exactly that reason.)
    s8_re = re.compile(
        r'Solver 8\n'
        r'  ! === SOLVER 8: WIRE-DIRECTION SOLVER ===.*?'
        r'  Linear System Residual Output = 1000\n'
        r'End\n',
        re.DOTALL)
    sif, n = s8_re.subn('Solver 8\n' + _COIL_SOLVER_BLOCK_COILSOLVER,
                       sif, count=1)
    if n != 1:
        raise SystemExit(
            "[err] CoilSolver path: could not find Solver 8 block to swap. "
            "Did the template change?")
    print('  [coilsolver] swapped Solver 8 (WPotentialSolver -> CoilSolver)')

    # ---- (2) trim Equation 1's Active Solvers, drop the 8 ----
    # Original line was `Active Solvers(10) = 5 6 7 8 1 9 2 3 10 11`.
    # New value: `Active Solvers(9) = 5 6 7 1 9 2 3 10 11` (no Solver 8).
    sif, n1 = re.subn(
        r'(?m)^(\s*)Active Solvers\(10\) = 5 6 7 8 1 9 2 3 10 11\s*$',
        r'\1Active Solvers(9) = 5 6 7 1 9 2 3 10 11',
        sif, count=1)
    if n1 != 1:
        raise SystemExit(
            "[err] CoilSolver path: could not trim Solver 8 from "
            "Equation 1 Active Solvers.")
    # Move Body 2 and Body 3 from Equation 1 to Equation 2.
    b2_pat = re.compile(r'(?ms)(^Body 2\n  Name[ ]*=[ ]*"Magnet"\n  )Equation = 1\n')
    sif, n2 = b2_pat.subn(r'\g<1>Equation = 2\n', sif)
    if n2 != 1:
        raise SystemExit(
            "[err] CoilSolver path: Body 2 (Magnet) could not be moved to "
            "Equation 2.")
    b3_pat = re.compile(r'(?ms)(^Body 3\n  Name[ ]*=[ ]*"AirDomain"\n  )Equation = 1\n')
    sif, n3 = b3_pat.subn(r'\g<1>Equation = 2\n', sif)
    if n3 != 1:
        raise SystemExit(
            "[err] CoilSolver path: Body 3 (AirDomain) could not be moved to "
            "Equation 2.")
    # Show the Eq 1 block we are about to match.
    em = re.search(r'(?ms)^Equation 1\n.*?End\n', sif)
    if em:
        print('[coilsolver] Eq 1 before:', repr(em.group(0)[:120]))
    # Append Equation 2 block + add the MainUtils.F90:1633 logical key on Eq 1.
    eq1_old = ('Equation 1\n  Name = "Magnetics"\n'
    ...
    eq1_new = ('Equation 1\n  Name = "Magnetics"\n'
               ...
               '  Mesh Update = Logical True\nEnd\n')
    if eq1_old in sif:
        sif = sif.replace(eq1_old, eq1_new, 1)
        print('  [coilsolver] added Equation 2, moved Body 2/3 onto it, '
              'added Wire direction logical key on Equation 1')
    else:
        # Maybe the active-solvers trim was not exactly that line; try a
        # more forgiving match using the same regex engine.
        tr = re.compile(
            r'(?m)Equation 1\n  Name = "Magnetics"\n'
            r'  Active Solvers\(9\) = 5 6 7 1 9 2 3 10 11\n'
            r'  Mesh Update = Logical True\nEnd\n')
        sif, n = tr.subn(
            'Equation 1\n  Name = "Magnetics"\n'
            '  Active Solvers(9) = 5 6 7 1 9 2 3 10 11\n'
            '  Mesh Update = Logical True\n'
        ...
        if n != 1:
            raise SystemExit(
                "[err] CoilSolver path: could not append Equation 2 + "
                "Wire direction key. Eq 1 block format unexpected.")
        print('  [coilsolver] (regex) added Equation 2 and Wire direction key')

    # ---- (3) strip Coil Start / Coil End (CoilSolver sets them itself) ----
    cs1_pat = re.compile(r'(?m)^\s*Coil Start = Logical True\s*\n')
    sif, n1 = cs1_pat.subn('', sif)
    cs2_pat = re.compile(r'(?m)^\s*Coil End = Logical True\s*\n')
    sif, n2 = cs2_pat.subn('', sif)
    if n1 != 1 or n2 != 1:
        raise SystemExit(
            "[err] CoilSolver path: expected exactly 1 Coil Start and 1 "
            "Coil End line, found %d / %d" % (n1, n2))
    # Also strip the `W = Real 1` / `W = Real 0` Dirichlet assignments
    # on BCs 3 / 4.  CoilSolver writes its own Dirichlet on the Electrode
    # Boundaries, so these become orphan keys -- harmless to Elmer, but
    # some parsers (and debug tools) get confused by stray `W = Real`
    # lines that are no longer inside the WPotentialSolver's domain.
    # Verified safe to remove (see hpc/notes.md 20.6).
    w1_pat = re.compile(r'(?m)^(  W = Real 1)\s*\n')
    sif, w1 = w1_pat.subn('', sif)
    w0_pat = re.compile(r'(?m)^(  W = Real 0)\s*\n')
    sif, w0 = w0_pat.subn('', sif)
    # The default BC 3 / BC 4 also set W = 1 / W = 0.  CoilSolver writes
    # its own Dirichlet on the Electrode Boundaries, so the leftover
    # `W = Real 1/0` lines become unused keys -- harmless, leave them.
    print('  [coilsolver] stripped Coil Start / Coil End from BCs 3/4')
...
# --- --path coilsolver support ----------------------------------------------

# Replaces Solver 8 wholesale inside _COIL_SOLVER_BLOCK.  Adapted from
# Elmerfem/tests/circuits_transient_stranded_full_coil/sif/coil.sif Solvers
# 1-4, with four adaptations for our radial-slit geometry (full reasoning
# in hpc/notes.md 20.2):
#   1. NO `Coil Closed = Logical True` -- that requires a cut-free closed
#      loop (upstream coil.sif:5-6), which our mesh does NOT have.  We DO
#      supply `Electrode Boundaries` on Component 1, so the cut-mode path
#      is fully specified.
#   2. CoilSolver.F90:358-359 Fatals if `Coil Normal` is also in the
#      SOLVER -- it must be on the COMPONENT (we put it there).
#   3. CoilSolver.F90:73 owns the Solver Variable (`-nooutput CoilTmp`),
#      so we DELETE the default `Variable = W` line.
#   4. CoilSolver.F90:385-388 Fatals if a body active in the Equation
#      carrying CoilSolver does not belong to a Component.  Bodies 2/3 go
#      to a new Equation 2 (handled by _apply_coilsolver_path).
#   5. CoilSolver.F90:130-152 owns Coil Start / Coil End (Electrode
#      Boundaries); strip the default BC 3 / BC 4 assignments.
_COIL_SOLVER_BLOCK_COILSOLVER = """
  Exec Solver = "Before All"
  Equation = "Wire direction"
...
def _r_wire(cfg: dict, curve: dict, n_turns: int) -> float:
    """Physical wire resistance R_wire = N * 2*pi*r_mean / (sigma_wire * A_wire).

    Used as `Component 1 Resistance = R_wire` on the --path coilsolver path,
    which gives r_component(1) = R_wire exactly (mesh-independent).
    Verified locally by setting Component 1 Resistance = 1.0e-3 and reading
    1.000e-3 from circuit.csv col 14 (`_identify_circuit_columns.py`).
    """
    coil = cfg['coil']
    r_mean = 0.5 * (float(coil['r_outer_m']) + float(coil['r_inner_m']))
    l_per_turn = 2.0 * math.pi * r_mean
    d = float(curve['wire_diameter_m'])
    a_wire = math.pi * (0.5 * d) ** 2
    sigma = float(curve['wire_conductivity_S_per_m'])
    return n_turns * l_per_turn / (sigma * a_wire)
...
def _insert_circuit_components(sif: str, curve: dict, cfg: dict) -> str:
    ...
    block = (_COMPONENT_BLOCK.format(
                ...
                n_j=n_j,
                r_wire=_r_wire(cfg, curve, n_turns))
...
def patch_circuit_closed(sif: str, curve: dict, cfg: dict,
                         coil_path: str = "wsolve") -> str:
    ...
    #
    # --path wsolve:   Material 1 is the actual conductor -- must carry the
    #   homogenised conductivity f * sigma_wire so the integral
    #       R_coil = N_j**2 * V_coil / sigma
    #   reproduces the physical wire resistance R_wire(N).  Without f, R
    #   is ~10x too small and the induced current is ~10x too large.
    # --path coilsolver: Material 1 is non-conductive (Dummy).  Elmer takes
    #   the resistance from `Component 1 Resistance = R_wire(N)` directly
    #   (the COMPONENT key is verified by `_identify_circuit_columns.py`).
    if coil_path == "coilsolver":
        sigma_eff = 1.0e-12
    else:
    ...
    # --path coilsolver: apply the four SIF rewrites the CoilSolver / W
    # vector path needs (Solver 8 swap, Equation split, logical key,
    # BC 3/4 strip).  Done LAST so it sees the final `Active Solvers` line
    # and the bodies' `Equation = N` after the closed-circuit insertions.
    if coil_path == "coilsolver":
        sif = _apply_coilsolver_path(sif, curve, cfg)
...
def main():
    ...
    ap.add_argument("--solver", choices=SOLVERS, default="umfpack",
                    help="linear-algebra backend for Solver 2 "
                         "(default: umfpack)")
    ap.add_argument("--path", choices=["wsolve", "coilsolver"],
                    default="wsolve",
                    help="Wire-direction solver: 'wsolve' (default, scalar W "
                         "via WPotentialSolver/Wsolve; needs DirectionSolver "
                         "+ RotMSolver, Material 1 sigma = f*sigma_wire) or "
                         "'coilsolver' (W vector via CoilSolver; coil body "
                         "is non-conductive, Component 1 Resistance = R_wire, "
                         "Equation 2 added for Bodies 2/3, 'Wire direction' "
                         "logical key on Equation 1).")
    ...
    if closed_circuit:
        ...
        new = patch_circuit_closed(new, c, cfg, coil_path=a.path)
    ...
    if closed_circuit:
        ...
        if a.path == "coilsolver":
            r_w = _r_wire(cfg, c, n)
            print(f"                   --path coilsolver: Solver 8 is CoilSolver (W vector)")
            print(f"                   --path coilsolver: Material 1 sigma = 1e-12 "
                  f"(non-physical Dummy)")
            print(f"                   --path coilsolver: Component 1 "
                  f"Resistance = {r_w:.6e} ohm (= R_wire(N))")
            print(f"                   --path coilsolver: Equation 2 added "
                  f"for Bodies 2/3 (no Solver 8)")
        ...
        print("                   VERIFIED: all 10 solvers run, the circuit "
              f"assembles, r_component(2) = {c['R_load_ohm']} ohm "
              "(= bench loop: series + shunt + leads).")
    elif a.curve:
        print(f"     curve       : {a.curve}  "
